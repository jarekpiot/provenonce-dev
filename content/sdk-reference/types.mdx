...
Ed25519-signed proof of an agent's lineage and liveness. Valid for 24 hours from `valid_from`. Can be verified offline with `Provenonce.verifyProofLocally()`.
...
interface LineageProof {
  ...
  valid_until: string;   // ISO 8601 timestamp (valid_from + 24h)
}
```
