...
- **Used for:** `init`, `heartbeat`, `purchaseSigil`, `reissueProof`, `spawn` endpoints
...
| `POST /api/v1/agent/purchase-sigil` | API key | Active (v0.9.0) |
...
Supported chains: Solana (Ed25519) and Ethereum (secp256k1). The private key is generated client-side and never sent to the server. Note: the key resides in the agent's process memory, which the host operator can access. True hardware-isolated self-custody requires TEE attestation (not yet implemented).
