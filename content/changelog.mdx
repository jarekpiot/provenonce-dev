...
- Upgraded authenticated route rate limiting from IP-only to multi-dimensional keys:
  endpoint path + authenticated subject, with path + IP fallback when subject is missing.
- Added fail-fast cost guards on paid routes so oversized/invalid payloads are rejected before auth/payment/DB work:
  `/api/v1/agent/spawn`, `/api/v1/agent/heartbeat`, `/api/v1/agent/reissue-proof`, `/api/v1/sigil` (commit `ebcf57c`).
- Added route-level fail-fast tests:
  `tests/cost-guards-fastfail.test.ts`, `tests/heartbeat-cost-guards-fastfail.test.ts`,
  `tests/reissue-proof-cost-guards-fastfail.test.ts`, `tests/sigil-cost-guards-fastfail.test.ts`.
- Added payment-failure throttling regression coverage to ensure repeated invalid/replayed payment attempts
  consume limiter budget and converge to `429` (`tests/payment-failure-throttling.test.ts`, commit `80046c3`,
  tag `security-hardening-p3-2`).
